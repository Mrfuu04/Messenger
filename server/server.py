...
class Server(
        ...
        with socket(AF_INET, SOCK_STREAM) as server_sock:
            ...
            while True:
                try:
                    ...
                    self.logger.info(
                        f'Подключен пользователь {client} {addr}'
                    )
                except TimeoutError:
                    pass
                else:
                    ...
                    if writers_list:
                        for writer in writers_list:
                            try:
                                ...
                                try:
                                    self.logger.warning(
                                        f'Ошибка при приеме сообщения от {writer}\n{e}'
                                    )
                    ...
                    for message in self.messages:
                        try:
                            ...
                            self.logger.warning(
                                f'Потеряно соединение с {message[SENDER]}'
                            )
    ...
    def process_client_message(self, client_sock):
        ...
        if message is None:
            response = RESPONSE_400
            response[MESSAGE_TEXT] = 'Ошибка при отправке сообщения'
            ...
            # Регистрация
                ...
                if registered == NICKNAME_IN_USE:
                    ...
                    self.logger.info(
                        f'Пользователь {message[SENDER]} зарегистрирован'
                    )
                else:
